...
class AntarcticaGenerator(DataGenerator):
    ...
    def load_sim_data(self, root_dir):
        ...
        # Load ERA5 data
        self.sim_data = {
            i : nc.Dataset(f"{root_dir}/tas_{i}.nc") for i in self.years
        }
    ...
    def load_real_data(self, root_dir):
        ...
        self.real_metadata = pd.read_csv(
    ...
    def generate_batch(self):
        ...
        with B.on_device(self.device):
            ...
            batch["real_x"] = np.stack(
                [array[:min_real_data] for array in batch["real_x"]],
                axis=0
            )

            batch["real_y"] = np.stack(
                [array[:min_real_data] for array in batch["real_y"]],
                axis=0
            )

            batch["real_temp"] = np.stack(
                [array[:min_real_data] for array in batch["real_temp"]],
                axis=0
            )
    ...
    def unpack_daily_sim_data(self, date):
        ...
        sim_temp = np.array(
            self.sim_data[int(date[:4])]["t2m"][d2-d1]
        ) - 273.15
